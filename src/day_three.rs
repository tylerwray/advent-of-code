...
fn build_points(path: &str) -> Vec<(i32, i32)> {
    let instructions = parse_path(path);
    let mut points = vec![(0, 0)];

    for i in instructions.iter() {
        match points.last() {
            Some(current_point) => {
                let mut line = draw_line(i, *current_point);
...
fn draw_line(instruction: &str, (x, y): (i32, i32)) -> Vec<(i32, i32)> {
    let mut line = Vec::new();
    let (direction, number) = parse_instruction(instruction);
...
fn parse_instruction(s: &str) -> (&str, &str) {
    for i in 1..5 {
        let r = s.get(0..i);
        match r {
            Some(x) => return (x, &s[i..]),
